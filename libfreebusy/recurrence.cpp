...
{
	if (m_sRecState.ulRecurFrequency != RF_MONTHLY && (m_sRecState.ulPatternType != 2 || m_sRecState.ulPatternType != 4))
		return 0;
...
ULONG recurrence::getModifiedMeetingType(ULONG id)
{
	if (id >= m_sRecState.ulModifiedInstanceCount)
		return 0;
	return m_sRecState.lstExceptions[id].ulMeetingType;
}
...
HRESULT recurrence::setModifiedMeetingType(ULONG id, ULONG type)
{
	...
	m_sRecState.lstExceptions[id].ulMeetingType = type;
