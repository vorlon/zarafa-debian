 ...
 * @return MAPI error code
 */
HRESULT Util::FileLFtoCRLF(FILE *fin, FILE** fout)
{
	HRESULT hr = hrSuccess;
	...
	while (!feof(fin)) {
		...
		Util::BufferLFtoCRLF(readsize, bufferin, bufferout, &sizebufferout);
 ...
 */
bool Util::DupFile(ECLogger *lpLogger, FILE *lpFile, std::string &strFileName) 
{
	bool bResult = true;
	size_t	ulReadsize = 0;
	FILE *pfNew = NULL;
 ...
 */
void Util::BufferLFtoCRLF(size_t size, const char *input, char *output, size_t *outsize) {
	size_t j = 0;
 ...
 */
void Util::StringTabtoSpaces(const std::wstring &strInput, std::wstring *lpstrOutput) {
 ...
 * @param[out] strOutput	return converted string
 */
void Util::StringCRLFtoLF(const std::wstring &strInput, std::wstring *lpstrOutput) {
	std::wstring::const_iterator iInput(strInput.begin());
	std::wstring strOutput;
 ...
 * @return 
 */
HRESULT Util::HrMapFileToString(FILE *f, std::string *lpstrBuffer, int *lpSize)
{
	HRESULT hr = hrSuccess;
	char *lpBuffer = NULL;
	int ulBufferSize = 0;
	...
	hr = Util::HrMapFileToBuffer(f, &lpBuffer, &ulBufferSize, &immap); // what if message was half read?
	if (hr != hrSuccess || !lpBuffer)
		goto exit;

	if (lpstrBuffer)
		*lpstrBuffer = string(lpBuffer, ulBufferSize);
	if (lpSize)
		*lpSize = ulBufferSize;

exit:
	if (lpBuffer)
		Util::HrUnmapFileBuffer(lpBuffer, ulBufferSize, immap);
 ...
 * @return MAPI error code
 */
HRESULT Util::HrMapFileToBuffer(FILE *f, char **lppBuffer, int *lpSize, bool *lpImmap)
{
	HRESULT hr = hrSuccess;
	char *lpBuffer = NULL;
	int offset = 0;
 ...
 */
void Util::HrUnmapFileBuffer(char *lpBuffer, int ulSize, bool bImmap)
{
	if (bImmap)
