...
BEGIN {
    ...
    # Import all symbols from MAPICore::
    my $sym;
    foreach $sym (keys %MAPICore::) {
        if($sym eq "isa") {
            next;
        }
        $MAPI::{$sym} =  $MAPICore::{$sym};
    }
}

END {
    MAPICore::MAPIUninitialize();
}
...
sub PpropFindProp($$)
{
    my ($propref, $ulPropTag) = @_;
    my $prop;
    
    foreach $prop (@$propref) {
...
sub DEFINE_GUID($$$$$$$$$$$) {
	my ($l, $w1, $w2, $b1, $b2, $b3, $b4, $b5, $b6, $b7, $b8) = @_;
	return pack("LS2C8", $l, $w1, $w2, $b1, $b2, $b3, $b4, $b5, $b6, $b7, $b8);
};
...
# FIXME: In UNICODE mode, switch to PT_UNICODE
use constant {
    PT_TSTRING		=> MAPICore::PT_STRING8,
    PT_MV_TSTRING	=> MAPICore::PT_MV_STRING8,
};

# Default properties
use constant {
    # Generated from
    # grep PR_ ../mapi4linux/include/mapitags.h | perl -e 'while(<>) { chomp(); if(/(PR_\S+).*(PROP_TAG\(.*\))/) { print "    " . $1 . " => " . $2 . ",\n"; }}'
...
use constant {
    # Generated from
    # grep PR_ ../common/windows/EdkMdb.h | perl -e 'while(<>) { chomp(); if(/(PR_\S+).*(PROP_TAG\(.*\))/) { print "    " . $1 . " => " . $2 . ",\n"; }}'
...
use constant {
    # From grep PR_ ../../common/mapiext.h | perl -e 'while(<>) { chomp(); if(/(PR_\S+).*(PROP_TAG\(.*\))/) { print "    " . $1 . " => " . $2 . ",\n"; }}'
...
use constant {
	# Generated from
	# grep DEFINE_OLEGUID ../mapi4linux/include/mapiguid.h | sed -e 's/^\(DEFINE_OLEGUID(\)\([^,]*\),\([^;]*\);\s$/    \2 => \1\3,/'
...
use constant {
    # Generated from
	# grep DEFINE_GUID ../common/edkguid.h | sed -e 's/^\(DEFINE_GUID(\)\([^,]*\),\([^;]*\);\s$/\2 => \1\3,/' | sed -e 's/^DEFINE_GUID(/# DEFINE_GUID(/'
...
my $sym;
# Build our exported variables by looping through all symbols defined previously
foreach $sym (keys %MAPI::) {
    # Skip subpackage symbols (eg MAPI::Zarafa::)
    if($sym =~ /::$/ || $sym eq "BEGIN") {
        next;
    }
    
    # Add all symbols as OK to export, and add them to the 'all' tag
    push @EXPORT_OK, $sym;
    push @{$EXPORT_TAGS{"all"}}, $sym;

    # Split the symbols into 'tags', 'restrictions' and 'flags'
    if($sym =~ /^PR_/) {
        push @{$EXPORT_TAGS{"tags"}}, $sym;
    }
    elsif($sym =~ /^RES_/) {
        push @{$EXPORT_TAGS{"restrictions"}}, $sym;
    }
    else {
        push @{$EXPORT_TAGS{"flags"}}, $sym;
    }
} 

package MAPI::Util;

sub CreateProfileAndConfigure($$$) {
    my ($profname, $service, $props) = @_;

    my $profadmin = MAPI::MAPIAdminProfiles(0);
    $profadmin->CreateProfile($profname,"",0,0);
    
    my $serviceadmin = $profadmin->AdminServices($profname, "", 0, 0);
    $serviceadmin->CreateMsgService($service, $service, 0, 0);
    
    my $msgservicetable = $serviceadmin->GetMsgServiceTable(0);
    my $rows = $msgservicetable->QueryRows(1, 0);

    if(@$rows != 1) {
        throw("not found");
    }

    my $provider_uid = MAPI::PpropFindProp($rows->[0], MAPI::PR_SERVICE_UID);

    if(!defined($provider_uid)) {
        throw("PR_SERVICE_UID not found");
    }
    
    $serviceadmin->ConfigureMsgService($provider_uid->{"Value"}, 0, 0, $props);
}
...
use constant {
    PR_EC_PATH 				=> MAPI::PROP_TAG(MAPI::PT_STRING8, PR_EC_BASE + 0x00),
    PR_EC_USERNAME 			=> MAPI::PROP_TAG(MAPI::PT_STRING8, PR_EC_BASE + 0x01),
    PR_EC_USERPASSWORD		=> MAPI::PROP_TAG(MAPI::PT_STRING8, PR_EC_BASE + 0x02),
    PR_EC_PORT 				=> MAPI::PROP_TAG(MAPI::PT_STRING8, PR_EC_BASE + 0x03),
    PR_EC_FLAGS				=> MAPI::PROP_TAG(MAPI::PT_STRING8, PR_EC_BASE + 0x04),
	PR_EC_SSLKEY_FILE		=> MAPI::PROP_TAG(MAPI::PT_STRING8,	PR_EC_BASE + 0x05),
	PR_EC_SSLKEY_PASS		=> MAPI::PROP_TAG(MAPI::PT_STRING8,	PR_EC_BASE + 0x06),
};

sub OpenECSession($$;$$$)
{
    my ($user, $pass, $server, $sslfile, $sslpass) = @_;
    my ($session);
	...
	$sslfile = '' unless(defined($sslfile));
	$sslpass = '' unless(defined($sslpass));
 
    my $props = [
        { "ulPropTag" => MAPI::Zarafa::PR_EC_PATH, "Value" => $server },
        { "ulPropTag" => MAPI::Zarafa::PR_EC_USERNAME, "Value" => $user },
        { "ulPropTag" => MAPI::Zarafa::PR_EC_USERPASSWORD, "Value" => $pass },
        { "ulPropTag" => MAPI::Zarafa::PR_EC_SSLKEY_FILE, "Value" => $sslfile },
        { "ulPropTag" => MAPI::Zarafa::PR_EC_SSLKEY_PASS, "Value" => $sslpass },
    ];
    
    TRY: {
        MAPI::Util::CreateProfileAndConfigure("prof", "ZARAFA6", $props);
        $session = MAPI::MAPILogonEx(0, "prof", "", 0);    }

    return $session;
}

package MAPI::Exception;
use Error;

@MAPI::Exception::ISA = qw(Error);
...
sub new {
    ...
    $self->SUPER::new(hr => $hr);
}

sub stringify {
    my $self = shift;
    return sprintf("MAPI Error 0x%08X at %s line %d.\n", $self->{"hr"}, $self->file, $self->line);
}


my $mapiinit = new MAPI::MAPIINIT_0();
$mapiinit->swig_ulVersion_set(MAPI::MAPI_INIT_VERSION);
$mapiinit->swig_ulFlags_set(MAPI::MAPI_MULTITHREAD_NOTIFICATIONS);

MAPI::MAPIInitialize($mapiinit);
