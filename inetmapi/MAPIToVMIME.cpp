...
// These are named properties that are mapped to fixed values in Zarafa, so
	...
	for (i = 0; i < cNames; i++) {
			...
			PROP_TYPE(lpPropArray[i].ulPropTag) == PT_STRING8)
		{
			...
			if ((str[0] == 'X' || str[0] == 'x') && str[1] == '-') {
				...
				// we still want to know that this mail was generated by zarafa in the x-mailer header from handleExtraHeaders
...
HRESULT MAPIToVMIME::handleTNEF(IMessage* lpMessage, vmime::messageBuilder* lpVMMessageBuilder, BOOL bRealRTF) {
	...
	std::list<ULONG>::iterator iterAttach;
