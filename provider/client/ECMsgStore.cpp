...
HRESULT ECMsgStore::SetProps(ULONG cValues, LPSPropValue lpPropArray, LPSPropProblemArray *lppProblems)
{
	...
	hr = ECMAPIProp::SaveChanges(KEEP_OPEN_READWRITE);
	if (hr != hrSuccess)
		goto exit;
...
HRESULT ECMsgStore::SetReceiveFolder(LPTSTR lpszMessageClass, ULONG ulFlags, ULONG cbEntryID, LPENTRYID lpEntryID)
{
	...
	hr = lpTransport->HrSetReceiveFolder(this->m_cbEntryId, this->m_lpEntryId, convstring(lpszMessageClass, ulFlags), cbEntryID, lpEntryID);
...
BOOL ECMsgStore::IsPublicStore()
{
	...
	if(CompareMDBProvider(&this->m_guidMDB_Provider, &ZARAFA_STORE_PUBLIC_GUID))
		fPublicStore = TRUE;
...
BOOL ECMsgStore::IsDelegateStore()
{
	...
	if(CompareMDBProvider(&this->m_guidMDB_Provider, &ZARAFA_STORE_DELEGATE_GUID))
		fDelegateStore = TRUE;
...
HRESULT ECMsgStore::AbortSubmit(ULONG cbEntryID, LPENTRYID lpEntryID, ULONG ulFlags)
{
	...
	hr = lpTransport->HrAbortSubmit(cbEntryID, lpEntryID);
...
HRESULT ECMsgStore::GetWrappedStoreEntryID(ULONG* lpcbWrapped, LPENTRYID* lppWrapped)
{
	...
	//hr = WrapStoreEntryID(0, ZARAFA_DLL_NAME, CbEID(peid), (LPENTRYID)peid, &cbWrapped, &lpWrapped);
	hr = lpSupport->WrapStoreEntryID(this->m_cbEntryId, this->m_lpEntryId, lpcbWrapped, lppWrapped);
	if (hr != hrSuccess)
		goto exit;
exit:
	return hr;
...
HRESULT ECMsgStore::CreateStoreEntryID(LPTSTR lpszMsgStoreDN, LPTSTR lpszMailboxDN, ULONG ulFlags, ULONG *lpcbEntryID, LPENTRYID *lppEntryID)
{
	...
	hr = WrapStoreEntryID(0, (LPTSTR)ZARAFA_DLL_NAME, cbStoreEntryID, lpStoreEntryID, lpcbEntryID, lppEntryID);
...
HRESULT ECMsgStore::CreateStore(ULONG ulStoreType, ULONG cbUserId, LPENTRYID lpUserId, ULONG* lpcbStoreId, LPENTRYID* lppStoreId, ULONG* lpcbRootId, LPENTRYID *lppRootId)
{
	...
	if(ulStoreType == ECSTORE_TYPE_PRIVATE) {
		memcpy(&lpecMsgStore->m_guidMDB_Provider, &ZARAFA_SERVICE_GUID, sizeof(MAPIUID));
	} else {
		memcpy(&lpecMsgStore->m_guidMDB_Provider, &ZARAFA_STORE_PUBLIC_GUID, sizeof(MAPIUID));
	}
...
HRESULT ECMsgStore::CreateEmptyStore(ULONG ulStoreType, ULONG cbUserId, LPENTRYID lpUserId, ULONG ulFlags, ULONG* lpcbStoreId, LPENTRYID* lppStoreId, ULONG* lpcbRootId, LPENTRYID* lppRootId)
{
	...
	if ((ulStoreType != ECSTORE_TYPE_PUBLIC && ulStoreType != ECSTORE_TYPE_PRIVATE) ||
...
HRESULT ECMsgStore::UnhookStore(ULONG cbUserId, LPENTRYID lpUserId)
{
	HRESULT	hr = hrSuccess;

	hr = lpTransport->HrUnhookStore(cbUserId, lpUserId, 0);
...
HRESULT ECMsgStore::GetStore(ULONG cbUserId, LPENTRYID lpUserId, ULONG *lpulStoreId)
{
	HRESULT			hr = hrSuccess;

	hr = lpTransport->HrGetStore(cbUserId, lpUserId, lpulStoreId);
...
HRESULT ECMsgStore::GetServerDetails(LPECSVRNAMELIST lpServerNameList, ULONG ulFlags, LPECSERVERLIST* lppsServerList)
{
	HRESULT hr = hrSuccess;
...
HRESULT ECMsgStore::DeleteFromMasterOutgoingTable(ULONG cbEntryId, LPENTRYID lpEntryId, ULONG ulFlags)
{
	...
	hr = this->lpTransport->HrFinishedMessage(cbEntryId, lpEntryId, EC_SUBMIT_MASTER | ulFlags);
...
HRESULT ECMsgStore::xMsgStore::OpenEntry(ULONG cbEntryID, LPENTRYID lpEntryID, LPCIID lpInterface, ULONG ulFlags, ULONG *lpulObjType, LPUNKNOWN *lppUnk) {
	TRACE_MAPI(TRACE_ENTRY, "IMsgStore::OpenEntry", "interface=%s, cb=%d, EntryId=%s", (lpInterface)?DBGGUIDToString(*lpInterface).c_str():"", cbEntryID, bin2hex(cbEntryID, (BYTE*)lpEntryID).c_str());
	METHOD_PROLOGUE_(ECMsgStore, MsgStore);
...
HRESULT ECMsgStore::xMsgStore::SetReceiveFolder(LPTSTR lpszMessageClass, ULONG ulFlags, ULONG cbEntryID, LPENTRYID lpEntryID) {
	TRACE_MAPI(TRACE_ENTRY, "IMsgStore::SetReceiveFolder", "");
	METHOD_PROLOGUE_(ECMsgStore, MsgStore);
...
HRESULT ECMsgStore::xECServiceAdmin::UnhookStore(ULONG cbUserId, LPENTRYID lpUserId)
{
	TRACE_MAPI(TRACE_ENTRY, "IECServiceAdmin::UnhookStore", "");
	METHOD_PROLOGUE_(ECMsgStore, ECServiceAdmin);
	HRESULT hr = pThis->UnhookStore(cbUserId, lpUserId);
...
HRESULT ECMsgStore::xECServiceAdmin::GetStore(ULONG cbUserId, LPENTRYID lpUserId, ULONG *lpulStoreId)
{
	TRACE_MAPI(TRACE_ENTRY, "IECServiceAdmin::GetStore", "");
	METHOD_PROLOGUE_(ECMsgStore, ECServiceAdmin);
	HRESULT hr = pThis->GetStore(cbUserId, lpUserId, lpulStoreId);
	TRACE_MAPI(TRACE_RETURN, "IECServiceAdmin::GetStore", "%s", GetMAPIErrorDescription(hr).c_str());
	return hr;
}
...
HRESULT ECMsgStore::xECServiceAdmin::GetServerDetails(LPECSVRNAMELIST lpServerNameList, ULONG ulFlags, LPECSERVERLIST* lppsServerList)
{
...
{
	TRACE_MAPI(TRACE_ENTRY, "IECServiceAdmin::GetPublicStore", "");
	METHOD_PROLOGUE_(ECMsgStore, ECServiceAdmin);
...
HRESULT ECMsgStore::xMsgStoreProxy::OpenEntry(ULONG cbEntryID, LPENTRYID lpEntryID, LPCIID lpInterface, ULONG ulFlags, ULONG *lpulObjType, LPUNKNOWN *lppUnk) {
	TRACE_MAPI(TRACE_ENTRY, "IMsgStoreProxy::OpenEntry", "interface=%s", (lpInterface)?DBGGUIDToString(*lpInterface).c_str():"");
	METHOD_PROLOGUE_(ECMsgStore, MsgStoreProxy);
	HRESULT hr = pThis->OpenEntry(cbEntryID, lpEntryID, lpInterface, ulFlags, lpulObjType, lppUnk);
...
{
	TRACE_MAPI(TRACE_ENTRY, "IMSLogon::OpenEntry", "interface=%s, cb=%d, EntryId=%s", (lpInterface)?DBGGUIDToString(*lpInterface).c_str():"", cbEntryID, bin2hex(cbEntryID, (BYTE*)lpEntryID).c_str());
	METHOD_PROLOGUE_(ECMSLogon, MSLogon);
	HRESULT hr = pThis->OpenEntry(cbEntryID, lpEntryID, lpInterface, ulFlags, lpulObjType, lppUnk);
