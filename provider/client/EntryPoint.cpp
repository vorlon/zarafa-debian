...
tstring		g_strCommonFilesZarafa;
tstring		g_strUserLocalAppDataZarafa;
tstring		g_strZarafaDirectory;
...
public:
    CZarafaApp() {
		...
		g_strManufacturer = _T("Zarafa");
		g_strProductName = _T("Zarafa Collaboration Platform");
		g_isOEM = false;
		g_ulLoadsim = FALSE;

		// FIXME for offline
		// - g_strUserLocalAppDataZarafa = ~/zarafa ?
		// - g_strZarafaDirectory = /usr/bin/ ?
    }
    ~CZarafaApp() {
 ...
 */
HRESULT GetServiceName(IProviderAdmin *lpProviderAdmin, std::string *lpServiceName)
{
...
HRESULT InitializeProvider(LPPROVIDERADMIN lpAdminProvider, IProfSect *lpProfSect, sGlobalProfileProps sProfileProps, ULONG *lpcStoreID, LPENTRYID *lppStoreID)
{
	...
	if(ulResourceType == MAPI_STORE_PROVIDER)
	{
		...
		if(CompareMDBProvider(ptrPropValueMDB->Value.bin.lpb, &ZARAFA_STORE_PUBLIC_GUID)) {
		...
		} else if(CompareMDBProvider(ptrPropValueMDB->Value.bin.lpb, &ZARAFA_STORE_DELEGATE_GUID)) {
		...
		hr = WrapStoreEntryID(0, (LPTSTR)ZARAFA_DLL_NAME, cbEntryId, ptrEntryId, &cbWrappedEntryId, &ptrWrappedEntryId);
		if(hr != hrSuccess) 
			goto exit;
		...
		sPropVals[cPropValue++].Value.lpszA = "Server";

		sPropVals[cPropValue].ulPropTag = PR_PROVIDER_DLL_NAME_A;
		sPropVals[cPropValue++].Value.lpszA = ZARAFA_DLL_NAME;
		...
		sPropVals[cPropValue++].Value.lpszA = "Zarafa Addressbook";
...
HRESULT UpdateProviders(LPPROVIDERADMIN lpAdminProviders, const sGlobalProfileProps &sProfileProps)
{
	HRESULT hr = hrSuccess;
